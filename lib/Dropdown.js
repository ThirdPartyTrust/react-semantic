...
var Dropdown = (function (_Component) {
  ...
  Dropdown.prototype.componentWillReceiveProps = function componentWillReceiveProps(nextProps) {
    if (nextProps.valid !== this.state.valid) {
      this.setState(_extends({}, this.state, { valid: nextProps.valid }));
    }
  };
  ...
  Dropdown.prototype.renderChild = function renderChild(child, bindChange, value) {
    if (bindChange === undefined) bindChange = true;
    ...
    if (Array.isArray(this.props.children)) {
      this.props.children.map((function (child) {
        if (child.props.value === _this4.props.defaultValue) {
          valueComponent = _this4.renderChild(child, false, _this4.props.defaultValue);
        }
      }).bind(this));
    } else {
  ...
  Dropdown.prototype.validate = function validate() {
    ...
    var isValid = !(this.props.require && !this.state.value);
    if (silent) {
      return isValid;
    }
    this.setState(_extends({}, this.state, {
